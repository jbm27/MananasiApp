...
const employees = rows
  .map((row) => {
    ...
    return {
      ...
      dateOfBirth: parseIsoDate(row['DATE OF BIRTH']),
      dateOfJoining: parseIsoDate(row['DATE OF JOINING']),
      ...
      bankBranch: String(row['BANK BRANCH'] ?? '').trim(),
      ...
      highestQualification: String(row['HIGHEST QUALIFICATION'] ?? '').trim(),
      relevantQualification: String(row['RELEVANT QUALIFICATION'] ?? '').trim(),
      emergencyContactName: String(row['EMERGENCY CONTACT NAME'] ?? '').trim(),
      emergencyContactRelation: String(row['EMERGENCY CONTACT RELATION'] ?? '').trim(),
      emergencyContactNumber: String(row['EMERGENCY CONTACT NUMBER'] ?? '').trim(),
    }
  })
  .filter(Boolean)
